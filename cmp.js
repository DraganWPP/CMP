/**
 * cmp.js - Joybringers Custom Branding v2.2
 * Custom Consent Management Platform
 * Designed for Joybringers Charity
 *
 * Works with GTM Custom Template.
 */
...
(function () {
  ...
  var CookieUtil = {
    ...
    remove: function (name) {
      ...
      document.cookie =
        name + '=;expires=Thu, 01 Jan 1970 00:00:00 UTC;path=/;SameSite=Lax' + secure;
    }
  };
  ...
  var StorageUtil = {
    save: function (data) {
      ...
      try {
        ...
        console.warn('[CMP] localStorage save failed', e);
      }
    },
    ...
    clear: function () {
      ...
      try {
        ...
        console.warn('[CMP] localStorage clear failed', e);
      }
    }
  };
  ...
  function updateGoogleConsent(categories, source) {
    ...
    console.log('[CMP] Updating consent from:', source, consent);
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      '#cmp-overlay{position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(3px);animation:cmpFadeIn .3s ease}',
  ...
  function showBannerFromSettings() {
    console.log('[CMP] show banner requested');
  ...
  function init() {
    ...
    if (stored && stored.version === CONFIG.consentVersion && stored.categories) {
      updateGoogleConsent(stored.categories, 'stored_consent');
      return;
    }

    console.log('[CMP] no valid stored consent, rendering banner');
  ...
  /*
   * Public functions.
   * These are needed because your GTM Custom Template calls:
  ...
  window.joybringersClearCmpConsent = function () {
    StorageUtil.clear();
    console.log('[CMP] consent cleared');
  };

  window.joybringersCmpDebug = function () {
    return {
      config: CONFIG,
      stored: StorageUtil.load(),
      showCmpBannerType: typeof window.showCmpBanner,
      joybringersShowCmpBannerType: typeof window.joybringersShowCmpBanner,
      dataLayerEvents: window.dataLayer.filter(function (x) {
        return x && (
          x.event === 'cmp_consent_update' ||
          x.event === 'cmp_consent_ready'
        );
      })
    };
  };
